...
function clean(text)
	...
	cleaned_text = filter(isinalphabet, map(lowercase, unaccent(text)))
...
unused_letters = alphabet[sample_freqs .== 0.0]
...
double_letters = alphabet[[sample_freq_matrix[i, i] 
					for i in 1:length(alphabet)] .!= 0.0]
...
most_likely_to_follow_w = alphabet[findmax(sample_freq_matrix[
									index_of_letter('w'), 1:end])[2]]
...
most_likely_to_precede_w = alphabet[findmax(sample_freq_matrix[
									1:end, index_of_letter('w')])[2]]
...
# ╔═╡ 96d7c80b-cd58-4538-b876-c693e1bf8757
matrix_sum = sum(col_sums)

# ╔═╡ cc62929e-f9af-11ea-06b9-439ac08dcb52
row_col_answer = md"""
We can interpret the row-wise sum as the percent of time a given letter precedes another letter, while the col-wise sum is the inverse.

The overall sum of the matrix is: $matrix_sum. This accounts for every two character combination.
"""
...
# ╔═╡ 9557ba85-9c3c-4b5e-ad79-11b830b8a47a
begin
	mytext = split("to be or not to be that is the question", " ")
	mylist = ngrams(mytext, 3)
end

# ╔═╡ b726f824-fb5e-11ea-328e-03a30544037f
function completion_cache(grams)
	n = length(grams[1])  # n in ngram
	
	cache = Dict()  # initialize dict
	for i in 1:length(grams)
		# check if key already exists
		if grams[i][1:n-1] in keys(cache)
			push!(cache[grams[i][1:n-1]], grams[i][n])
		else
			cache[grams[i][1:n-1]] = [grams[i][n]]
		end
	end
...
# ╟─00115b6e-f381-11ea-0bc6-61ca119cb628
# ╟─00000000-0000-0000-0000-000000000001
# ╟─00000000-0000-0000-0000-0000000000�v'