...
function finite_difference_slope(f::Function, a, h=1e-6)
...
# ╔═╡ 5941b8e1-1aa4-428d-be46-7e871fba865c
md"""
> When $h$ gets too small, there is floating point precision error that can impact the accuracy of our result.
"""
...
function euler_SIR(β, γ, sir_0::Vector, T::AbstractRange)
	...
	h = step(T)
	num_steps = length(T)
	sir_t = sir_0  # generalize sir_0
	
	sir_fs = []
	for i=1:num_steps
		sir_t = euler_SIR_step(β, γ, sir_t, h)
		push!(sir_fs, sir_t)
	end
...
> There is an outbreak that is observable that follows the described shape; however, the rate of infection seems more linear than exponential. Not everyboday ends up getting infected, only about 75 percent of the population.
"""
...
# ╔═╡ 6203b50e-4be9-4d14-89e2-adb4c2ee7af4
md"""
β = $@bind β_slider Slider(0:0.1:1, show_value=true, default=0.7)
"""

# ╔═╡ 4075a520-aa4d-4e9f-89c8-00e59580f981
md"""
γ = $@bind γ_slider Slider(0:0.1:1, show_value=true, default=0.2)
"""

# ╔═╡ 68274534-1103-11eb-0d62-f1acb57721bc
let
	sir_results = euler_SIR(β_slider, γ_slider, 
	[0.99, 0.01, 0.00], sir_T)
	plot_sir!(plot(), sir_T, sir_results)
end

# ╔═╡ d69e957f-f09e-4a2f-8241-3a0a688d86f5
md"""
> For an epidemic outbreak to occur, $β$ must be sufficiently large and $γ$ must be sufficiently small.
"""
...
function gradient_descent_1d_step(f, x0; η=0.01)
	...
	x = finite_difference_slope(f, x0)
...
function gradient_descent_1d(f, x0; η=0.01, N_steps=1_000, diff=1e-6)
	
	x = x0  # initialize x equal to x0
	for t=1:N_steps
		x0 = x
...
> I added a `diff` parameter above to trigger when to exit the loop. When the step size becomes smaller than the `diff` specified, the algorithm is considered to be converged and the programs exits.
"""
...
function gradient_descent_2d_step(f, x0, y0; η=0.01)
	x, y = -η * gradient(f, x0, y0)
	return (x0 + x, y0 + y)
end

# ╔═╡ 8a114ca8-12e8-11eb-2de6-9149d1d3bc3d
function gradient_descent_2d(f, x0, y0; η=0.01, n_steps=1_000)
...
function gradient_2d_viz_3d(N_gradient_2d, x0, y0)
	...
	plot!(p, first.(all), last.(all), trace, 
		color="blue", opacity=range(.5,step=.2,length=length(all)), 
		label=nothing, leg=false)
...
# ╔═╡ fcd38568-d51e-4489-857c-42c19e6d167e
md"""
> Yes, you can find different minima using the `gradient_descent_2d` function as it finds the local minimum solution, which depends on the starting position of $x_0$ and $y_0$. This algorithm, as it is currently written, does not solve for the global minimum.
"""
...
function loss_dice(μ, σ)
	offset = dice_x[1] - 1
	loss = sum((gauss(x, μ, σ) - dice_y[x - offset])^2 for x=dice_x)
	return loss
end
...
found_μ, found_σ = gradient_descent_2d(loss_dice, 30, 1, η=2)
...
# ╔═╡ 8567d645-8669-4bc7-aba1-9d0bb89f869a
sq_error(x::AbstractVector, y::AbstractVector) = [(x[i] - y[i])^2 for i=1:length(x)]

# ╔═╡ 754b5368-12e8-11eb-0763-e3ec56562c5f
function loss_sir(β, γ)
	sir0 = spatial_results[1]  # initial condition

	# compute ode sir results
	ode_results = [sir0]
	ode = euler_SIR(β, γ, sir0, spatial_T)
	append!(ode_results, ode)

	sq_errors = [sq_error(ode_results[i], spatial_results[i]) for i=spatial_T]

	return sum(sum(sq_errors))
end
...
found_β, found_γ = gradient_descent_2d(loss_sir, 0.05, 0.005, η=1e-8)
