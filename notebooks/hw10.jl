...
> A more negative value of $B$ lessens the impact of changes in CO₂. An equilibrium state is achieved more rapidly. It is known as the *climate feedback parameter* because it magnifies the impact of CO₂ in the atmosphere.
"""
...
> When *B* is greater than or equal to zero, the slope of the ECS curve is linear or exponential and does not seem to reach an equilibrium. This means that the climate will continue to warm infinitely.
"""
...
let
	bs = -1:0.01:1
	ecs = [ECS(B=b) for b in bs]

	plot(bs, ecs, leg=false)
	xlabel!("B")
	ylabel!("ECS")
end
...
histogram(ECS_samples, size=(600, 250), xlabel="ECS", 
		  ylabel="samples", leg=false)
...
begin
	mean_of_ecs = mean(ECS_samples)
	mean_of_b_ecs = ECS(B=mean(B_samples))
	p_ecs = count(>=(mean_of_b_ecs), ECS_samples) / length(ECS_samples)
	
	md"""
	  $\overline{\text{ECS}(B)}$ = $(round(mean_of_ecs; digits=2))  
	
	  $\text{ECS}(\overline{B})$ = $(round(mean_of_b_ecs; digits=2)) 

	  $P(\text{ECS}(B) > \text{ECS}(\overline{B}))$ = $(round(p_ecs; digits=2))
	"""
end
...
> There is less implied warming when accounting for the uncertainty in $B$. The majority of the distribution of ECS is between 1 and 3°C.
"""
...
begin
	plot(simulated_rcp85_model.t, simulated_rcp85_model.T, xlabel="year t",
	ylabel="temperature T (°C)", title="RCP8.5 Scenario", leg=false)
end

# ╔═╡ ee771045-1b1e-4c2e-a91f-3457630578ec
md"""
Temperature (°C) at year 2100 = $(round(simulated_rcp85_model.T[end]; digits=2))
"""
...
plot(t, Model.CO2_RCP85.(t), 
	ylim=(0, 1200), ylabel="CO2 concentration [ppm]", leg=false)

# ╔═╡ 50ea30ba-25a1-11eb-05d8-b3d579f85652
expected_double_CO2_year = let
	co2 = Model.CO2_RCP85.(t)
	double_co2 = co2[1] * 2
	findfirst(>=(double_co2), co2) + t[1]
end
...
# ╔═╡ b77f50d7-22c8-482f-9b62-13b4bc0f47e3
begin
	rcp26 = [temperature_response(Model.CO2_RCP26, b) - 14 for b in B_samples]
	rcp85 = [temperature_response(Model.CO2_RCP85, b) - 14 for b in B_samples]
	
	rcp26_prob = count(>(2), rcp26) / length(rcp26)
	rcp85_prob = count(>(2), rcp85) / length(rcp85)

	md"""
	There is a $(round(rcp26_prob * 100; digits=1))% probability that we will see more than 2°C of warming by 2100 under the low-emissions scenario RCP2.6. That answer jumps to $(round(rcp85_prob * 100; digits=1))% under the high-emissions scenario RCP8.5.
	"""
end
...
md"""
Log CO₂ Level $(@bind CO2_slider  Slider(1:0.01:10, show_value=true, default=2))
"""
...
# ╔═╡ 385f34bb-57d2-43a4-a775-db1316fdd000
begin
	ntraj = 10
...
# ╔═╡ 75188417-b6cf-429c-83a3-87b2abd23a04
...
let
	...
	plot_trajectory!(p, reverse(CO2traj), reverse(Ttraj), lw=9)
...
# ╔═╡ b09a826b-1dd0-4152-a369-1d54bd31c8c9
begin
	CO2_slider
	step_model!(ebm, 10^CO2_slider)

	insert!(CO2traj, 1, copy(10^CO2_slider))
	pop!(CO2traj)
...
# ╔═╡ 731bc827-8117-47ad-8f22-d03029ff96ae
function equilibrium_temp(CO2::Real)
	ebm = Model.EBM(-48., 0., 1., t -> CO2)
	Model.run!(ebm, 500)
	ebm.T[end]
end

# ╔═╡ ccf6a89b-ad5f-4c11-841f-2bfbdc0ced3b
for i=100:100:1_000_000
	temp = equilibrium_temp(i)
	if temp >= 10
		global co2_to_melt_snowball = i
		break
	end
end
...
md"""
The lowest concentration of CO₂ needed to melt the Snowball is $(co2_to_melt_snowball)."""
