...
function trajectory(w::Coordinate, n::Int)
	steps = rand(possible_moves, n)
	
	return accumulate(+, steps, init=w)
end
...
function collide_boundary(c::Coordinate, L::Number)
	...
	x = min(abs(c.x), L) * sign(c.x)
	y = min(abs(c.y), L) * sign(c.y)
...
function trajectory(c::Coordinate, n::Int, L::Number)
	...
	return accumulate((x, y) -> collide_boundary((x + y), L), steps, init=c)
end
...
let
	...
	for i=1:10
		t = trajectory(Coordinate(0,0), 1000)
		plot_trajectory!(p, t)
	end
...
# ╔═╡ d9873520-57cc-4b92-b85c-6ecb348701c8
let
	p = plot()
	plot_trajectory!(p, trajectory(Coordinate(0,0), 1_000, 10))
end
...
# ╔═╡ b23ddb3c-60fa-4953-b98d-32a48b928589
md"""
> Before we get started, let's copy some useful functions from Homework 7.
"""

# ╔═╡ 3de571fb-41ca-4a96-95bf-bab1d0197080
...
Every time that you move the slider, a completely new simulation is created an run. This makes it hard to view the progress of a single simulation over time. So in this exercise, we will look at a single simulation, and plot the S, I and R curves.
...
# ╔═╡ 6c841d6a-9cca-4083-a369-12e6ce3be5a5
md"""
p infected = $(@bind prob_infected Slider(0:0.01:1, default=0.25, show_value=true))
"""

# ╔═╡ 4e5c1716-9f47-4037-8ad0-5ee3c2481bb2
md"""
p recovery = $(@bind prob_recovery Slider(0:0.00001:0.1, default=0.00005, show_value=true))
"""

# ╔═╡ 63dd9478-0a45-11eb-2340-6d3d00f9bb5f
causes_outbreak = CollisionInfectionRecovery(0.75, 1e-5)

# ╔═╡ 269955e4-0a46-11eb-02cc-1946dc918bfa
does_not_cause_outbreak = CollisionInfectionRecovery(0.25, 0.001)
...
> Since the prior homework assignment used a *well-mixed* model, the spread of the infection did not depend on the position of the agents. We are now using a *spatial* model, so there are fewer opportunities for potential infections. To create an outbreak, the $p_{infection}$ must be signficantly higher than in the *well-mixed* model, while the $p_{recovery}$ needs to be much lower. This increases the length of time the agents carry the infection while making the disease much more contagious.
"""
...
# ╔═╡ 56d5b1cc-bc28-45fb-87c0-141f10e0262f
...
function initialize(N::Number, L::Number)
	...
	# create list of susceptible agents with random position
	agents = [Agent(Coordinate( rand(-L:L), rand(-L:L)), S, 0) for i=1:N]
	agents[rand(1:N)].status = I  # randomly select one agent to be infected
...
# ╔═╡ 7d39dd77-df08-472b-ac79-2fed140fce57
function set_status!(agent::AbstractAgent, new_status::InfectionStatus)
	agent.status = new_status
end

# ╔═╡ 5942dde2-f5f2-4d3b-89b6-32bb8f689902
function set_num_infected!(agent::AbstractAgent)
	agent.num_infected += 1
end

# ╔═╡ 9ffa14a2-b402-47db-bee8-81d43697fc19
function is_susceptible(agent::AbstractAgent)
	return agent.status == S
end

# ╔═╡ a187f91f-98f4-42d4-94f7-dcce464c6c81
function is_infected(agent::AbstractAgent)
	return agent.status == I
end

# ╔═╡ 7baaf290-ae85-436f-aca8-93737cc206dc
function is_recovered(agent::AbstractAgent)
	return agent.status == R
end

# ╔═╡ d56e49d1-9331-408e-ba39-a9c82f397e16
function is_same_position(agent::AbstractAgent, source::AbstractAgent)
...
function interact!(agent::Agent, source::Agent, infection::CollisionInfectionRecovery)
	...
	if is_susceptible(agent) && is_infected(source) && is_same_position(agent, source) 
		if bernoulli(infection.p_infection)
			set_status!(agent, I)
			set_num_infected!(source)
		end
...
function initialize_social(N, L)
	social = LinRange(0.1, 0.5, 10)
	
	# create list of susceptible agents with random position
	agents = [SocialAgent(Coordinate( rand(-L:L), rand(-L:L) ), S, 0, rand(social)) for i=1:N]
	agents[rand(1:N)].status = I  # randomly select one agent to be infected
...
function interact!(agent::SocialAgent, source::SocialAgent, infection::CollisionInfectionRecovery)
	...
	if is_susceptible(agent) && is_infected(source) && is_same_position(agent, source) 
		if bernoulli(agent.social_score + source.social_score) && bernoulli(infection.p_infection)
			set_status!(agent, I)
			set_num_infected!(source)
		end
...
# ╔═╡ ece27277-9814-4618-ac4f-1846f0bdb3c3
md"""
#### Exercise 4.3
...
# ╔═╡ b277df6b-7df1-41d4-8064-12c8cb0a3710
md"""
> There is generally a positive correlation between `social_score` and `num_infected`. This makes intuitive sense, as the more likely an individual is to socialize with others in close proximity, the higher the probability of infection becomes.
"""
...
# ╔═╡ 1c03d875-29d8-4e2f-941b-cfe4401f7a06
md"""
lockdown constant = $(@bind lockdown_constant Slider(0:0.05:2, default=0.25, show_value=true))
"""

# ╔═╡ d56b1c61-828d-4316-9879-cfcd6c0fdaf4
md"""
> The `lockdown_constant` seems to lower the spread of the infection; however, there is not a sharp decline right at $t$=100 because we are not reducing the agents who are currently infected. Instead, there seem to be essentially no new outbreaks, with the infected agents recovering gradually based on $p_{recovery}$. When the constant exceeds 1, agents are more social and we continue to see many new infections after $t$=100.
"""
...
# ╔═╡ 27a4f8be-8a02-431e-b44a-e3217dc56e95
md"""
> To start, let's define some new functions that take a new type `LockdownAgent` as an input.
"""

# ╔═╡ d3c5b243-fb16-4147-980a-b2ae2b697881
mutable struct LockdownAgent <: AbstractAgent
	position::Coordinate
	status::InfectionStatus
	initial_infected_position::Coordinate
end

# ╔═╡ 004ab8ac-adb5-4014-aee1-b8a17e59c2df
struct CollisionInfectionMovement <: AbstractInfection
	p_infection::Float64
	p_movement::Float64
end

# ╔═╡ 25a7e28e-02e3-4858-a7bf-dc7f3afce361
begin
	position(a::LockdownAgent) = a.position
	color(a::LockdownAgent) = color(a.status)
end

# ╔═╡ 1ccc961e-0a69-11eb-392b-915be07ef38d
function visualize(agents::Vector, L)
	
	xs = [agent.position.x for agent in agents]
	ys = [agent.position.y for agent in agents]

	lim = ceil(L * 1.1) # make plot lims slghtly larger
						# than L to handle edge cases
	p = scatter(xs, ys, c=color.(agents), ratio=1,
		xlims=(-lim, lim), ylims=(-lim, lim), leg=false)
...
# ╔═╡ 1d923bad-8f10-40af-8279-efc3b4924d19
function initialize_lockdown(N, L)
	social = LinRange(0.1, 0.5, 10)
	
	# create list of susceptible agents with random position
	agents = [LockdownAgent(Coordinate( rand(-L:L), rand(-L:L) ), S, Coordinate(0,0)) for i=1:N]
	agents[rand(1:N)].status = I  # randomly select one agent to be infected
	
	return agents
end

# ╔═╡ ee0524cb-d932-4f1e-8bc2-70fa65e56ea5
function interact!(agent::LockdownAgent, source::LockdownAgent, infection::CollisionInfectionMovement)
	
	if is_susceptible(agent) && is_infected(source) && is_same_position(agent, source) 
		if bernoulli(infection.p_infection)
			set_status!(agent, I)
			agent.initial_infected_position = agent.position
		end
	end
...
function step!(agents::Vector, L::Number, infection::AbstractInfection)
	...
	agent_ind = filter(!=(source_ind), 1:length(agents))
...
# ╔═╡ cac657ac-3943-4ce5-9ad6-fbfb597035bb
function step!(agents::Vector{SocialAgent}, L::Number, infection::AbstractInfection)
...
function step!(agents::Vector{LockdownAgent}, L::Number, infection::CollisionInfectionMovement)

	# handle infected movement
	infected_ind = rand(filter(!=(nothing),
						  [if agent.status == I ind end 
						   for (ind, agent) in enumerate(agents)]))
	infected = agents[infected_ind]
	infected.position = collide_boundary(infected.position + rand(possible_moves), L)

	# handle other agent movement
	other_ind = filter(!=(infected_ind), 1:length(agents))
	other_agent = agents[rand(other_ind)]
	if bernoulli(infection.p_movement)
		intended_step = collide_boundary(other_agent.position + rand(possible_moves), L)
		if !(intended_step in [i.position for i in agents])
			other_agent.position = intended_step
		end
	end
		
	# with movement settled, the agents are free to interact
	for i in other_ind
		interact!(agents[i], infected, infection)
	end

	return agents
end

# ╔═╡ 39653c47-7149-4a0b-b67a-ca22b6914d5e
...
let
	...
	title!(plot_after, "after")
	
	plot(plot_before, plot_after)
end

# ╔═╡ 24452f73-a9be-4f27-8b3b-9489f90992d0
function simulation(N::Integer, L::Number, T::Integer, infection::AbstractInfection)

	agents = initialize(N, L)

	# initialize dict with values at timestep 1
	stats = Dict("S" => [N-1], "I" => [1], "R" => [0], 
				"num_infected" => [repeat([0], N)])
	for i=2:T
		sweep!(agents, L, infection)
		push!(stats["S"], length(filter(is_susceptible, agents)))
		push!(stats["I"], length(filter(is_infected, agents)))
		push!(stats["R"], length(filter(is_recovered, agents)))
		push!(stats["num_infected"], [agent.num_infected for agent in agents])
	end
	
	return (S=stats["S"], I=stats["I"], R=stats["R"],
...
let
	...
	result = plot(1:T, sim.S, ylim=(0, N), label="Susceptible", leg=:left)
	plot!(result, 1:T, sim.I, ylim=(0, N), label="Infectious")
	plot!(result, 1:T, sim.R, ylim=(0, N), label="Recovered")

end

# ╔═╡ 6a34f4ac-8224-4e19-885f-126b279d3b6a
let
	N = 100
	L = 20
	T = 1_000
	infection = CollisionInfectionRecovery(prob_infected, prob_recovery)

	sim = simulation(N, L, T, infection)
	result = plot(1:T, sim.S, ylim=(1, N), label="Susceptible", leg=:left)
	plot!(result, 1:T, sim.I, ylim=(0, N), label="Infectious")
	plot!(result, 1:T, sim.R, ylim=(0, N), label="Recovered")
...
# ╔═╡ 42c803f8-24c2-4cfb-a6ad-5e867e0d6a4b
sims = repeat_simulations(50, 40, 10_000, pandemic, 50)

# ╔═╡ 18b2a584-d2bd-451b-b090-74d23ee9677e
begin
	N = 50
	T= 10_000
	result = plot(leg=false)
	for sim in sims
		plot!(result, 1:T, sim.S, ylim=(1, N), color=:blue, alpha=0.2)
		plot!(result, 1:T, sim.I, ylim=(1, N), color=:red, alpha=0.2)
		plot!(result, 1:T, sim.R, ylim=(1, N), color=:green, alpha=0.2)
	end
	result
end
...
let
	...
	@gif for t in 1:Tmax
		...
		push!(Ss, length(filter(is_susceptible, x)))
		push!(Is, length(filter(is_infected, x)))
		push!(Rs, length(filter(is_recovered, x)))
...
let
	N = 50
	L = 40
	social_infection = CollisionInfectionRecovery(0.4, 1e-6)
	...
	@gif for t in 1:Tmax
		for i in 1:50N
			step!(social_agents, L, social_infection)
		end

		push!(Ss, length(filter(is_susceptible, social_agents)))
		push!(Is, length(filter(is_infected, social_agents)))
		push!(Rs, length(filter(is_recovered, social_agents)))
...
let
	xs = [agent.social_score for agent in social_agents]
	ys = [agent.num_infected for agent in social_agents]

	b, m = [ one.(xs) xs]\ ys
	ex = (xs .* m ) .+ b
	
	p = scatter(xs, ys, alpha=0.2, leg=false)
	plot!(p, xs, ex)
end

# ╔═╡ ef2e5eab-a02f-41ac-9772-f261978a2cbd
let
	N = 100
	L = 40
	social_infection = CollisionInfectionRecovery(0.4, 1e-6)

	social_agents2 = initialize_social(N, L)
	Ss, Is, Rs = [], [], [], []
	...
	@gif for t in 1:Tmax
		...
		# initiate lockdown constant
		if t == 101
			for agent in social_agents2
				agent.social_score *= lockdown_constant
			end
		end
		
		for i in 1:50N
			step!(social_agents2, L, social_infection)
		end
		
		push!(Ss, length(filter(is_susceptible, social_agents2)))
		push!(Is, length(filter(is_infected, social_agents2)))
		push!(Rs, length(filter(is_recovered, social_agents2)))
...
let
	N = 50
	L = 40
	infection = CollisionInfectionMovement(0.5, 0.01)

	x = initialize_lockdown(N, L)
...
# ╔═╡ 1b16e7c2-ac85-4c7d-bf81-a6239874a890
md"""
> Since the infected agents are the only members of the system who are free to move at each timestep, they are driving the disease spread. In other words, the uninfected cannot come in contact with an infected agent, the infected agent must come to them.
"""
...
# ╔═╡ b8616434-ba53-4f0c-913f-8f6ea6628bf3
let
	N = 50
	L = 40
	infection = CollisionInfectionMovement(0.5, 0.1)

	lockdown_agents = initialize_lockdown(N, L)
	
	Tmax = 200
	for t in 1:Tmax
		for i in 1:50N
			step!(lockdown_agents, L, infection)
		end
	end

	xs = [agent.position.x for agent in lockdown_agents]
	ys = [agent.position.y for agent in lockdown_agents]
	scatter(xs, ys, leg=false, color=:blue, alpha=0.2)
	xlabel!("x position")
	ylabel!("y position")
end
...
# ╔═╡ 7335be93-91ca-495e-abed-ef187607c6e1
md"""
N = $(@bind N_slider Slider(0:10:1000, default=50, show_value=true))
"""

# ╔═╡ 2dcca0a2-9679-44dd-9331-00915b8d32a0
md"""
L = $(@bind L_slider Slider(0:10:200, default=100, show_value=true))
"""

# ╔═╡ 4e8531ea-09e7-4bad-83e6-0deccd7be46d
md"""
###### ρ = $(N_slider / L_slider^2)
"""
...
# ╔═╡ 2351310b-f552-4479-9887-85d7c6c398f1
md"""
> As the density $ρ$ increases, the number of locations where infections occurs magnifies. Additionally, the are many more individual coordinates as well as clusters of coordinates where repeat infections occur.
"""
...
# ╔═╡ 22e5629b-ff4a-4ef1-8752-0cac10ae5bdb
md"""
> Independent of $p_{infection}$ and $p_{movement}$, the density $ρ$ has a significant impact on the dynamics of infection in this leaky model. With a very small $ρ$, there is more space for a smaller number of agents to move around, prolonging the time until the entire population is infected, or flattening the curve. Conversely, when $ρ$ is quite large, the entire population is almost immediately infected. This makes intuitive sense: in areas where that are more densely populated, infections spread quicker.
"""
