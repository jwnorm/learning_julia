...
function greedy_seam(energies, starting_pixel::Int)
	...
	for i in 1:m-1
		...
		# check for bounds of matrix
		slice = max(next-1, 1):min(next+1, n)
		
		ind = findmin(energies[i+1, slice])[2]
		append!(seam, slice[ind])
	end
...
## returns lowest possible sum energy at pixel (i, j), 
## and the column to jump to in row i+1.
function least_energy(energies, i, j)
	...
	## induction
	j1, j2 = max(1, j-1), min(n, j+1)
	
	left = least_energy(energies, i+1, j1)
	center = least_energy(energies, i+1, j)
	right = least_energy(energies, i+1, j2)

	energy, ind = findmin([left, center, right])
	return (energy[1] + energies[i, j], [j1, j, j2][ind])
...
function recursive_seam(energies, starting_pixel)
	...
	for i in 1:m-1
		next = least_energy(energies, i, seam[end])[2]
		append!(seam, next)
	end
...
The algorithm does an exhaustive search of all possible paths because it recursively calls itself for lower left, down, and lower right cells, then repeats itself starting at the new position.

The number of possible seams grows by n^3 as the width of image increases.
"""
...
function memoized_least_energy(energies, i, j, memory::Dict)
	...
	left = memoized_least_energy(energies, i+1, j1, memory)
	center = memoized_least_energy(energies, i+1, j, memory)
	right = memoized_least_energy(energies, i+1, j2, memory)
	
	energy, ind = findmin([left, center, right])
	result = (energy[1] + energies[i, j], [j1, j, j2][ind])
	memory[i, j] = result
...
function memoized_recursive_seam(energies, starting_pixel)
	...
	seam = [starting_pixel]
	for i in 1:m-1
		next = memoized_least_energy(energies, i, seam[end], memory)[2]
		append!(seam, next)
	end
...
function matrix_memoized_least_energy(energies, i, j, memory::Matrix)
	...
	left = matrix_memoized_least_energy(energies, i+1, j1, memory)
	center = matrix_memoized_least_energy(energies, i+1, j, memory)
	right = matrix_memoized_least_energy(energies, i+1, j2, memory)
...
# ╔═╡ f4e6c593-ede4-4ca1-8dfc-24b68dd3180b
md"Let's see if our matrix result is equivalent to our dictonary result from the previous section."


# ╔═╡ 5264bcc5-e873-41b3-87ca-26fae4385195
begin
	memo_matrix = Matrix{Union{Nothing,Tuple{Float64,Int}}}(nothing, size(grant_example))
	
 	matrix_memoized_least_energy(grant_example, 1, 4, memo_matrix) ==
	memoized_least_energy_test
end
...
function matrix_memoized_seam(energies, starting_pixel)
	...
	seam = [starting_pixel]
	for i in 1:m-1
		next = matrix_memoized_least_energy(energies, i, seam[end], memory)[2]
		append!(seam, next)
	end
...
function least_energy_matrix(energies)
	...
	for i in m:-1:2
		for j in 1:n
			j1, j2 = max(1, j-1), min(n, j+1)
			least = minimum(energies[i, j1:j2])
			result[i-1, j] += least
		end
	end
...
function seam_from_precomputed_least_energy(energies, starting_pixel::Int)
	...
	# since we can establish a matrix pointing to the optimal path
	# based on any (i, j) position, we can use our greedy algo
	seam = greedy_seam(least_energies, starting_pixel)
