...
# ╔═╡ cf8690ae-b827-4df4-8c40-f7859e01e32d
function get_greens(image::AbstractMatrix)
	greens = get_green.(image)
	return greens
end

# ╔═╡ 96ca5126-2b5c-41be-a08f-d806594d50e7
function get_blue(pixel::AbstractRGB)
	blue = pixel.b
	return blue
end

# ╔═╡ 7bc6464a-a1ef-44bd-9835-b2d8f289e3e6
function get_blues(image::AbstractMatrix)
	blues = get_blue.(image)
	return blues
end
...
function mean_color(image)
	...
	mean_color = RGB(mean_red, mean_green, mean_blue)
...
function invert(color::AbstractRGB)
	...
	inv_color = RGB(inv_red, inv_green, inv_blue)
	return inv_color
end
...
function quantize(color::AbstractRGB)
	q_red = floor(color.r, digits=1)
	q_green = floor(color.g, digits=1)
	q_blue = floor(color.b, digits=1)

	q_color = RGB(q_red, q_green, q_blue)
	return q_color
end
...
function quantize(image::AbstractMatrix)
	q_reds = floor.(get_reds(image), digits=1)
	q_greens = floor.(get_greens(image), digits=1)
	q_blues = floor.(get_blues(image), digits=1)

	q_image = [RGB(r, g, b) for (r, g, b) in zip(q_reds, q_greens, q_blues)]
...
function noisify(x::Number, s)
	output = x + rand((-s, s))
	output_clamped = clamp(output, 0, 1)
...
function noisify(color::AbstractRGB, s)
	noisy_red = noisify(color.r, s)
	noisy_green = noisify(color.g, s)
	noisy_blue = noisify(color.b, s)

	noisy_color = RGB(noisy_red, noisy_green, noisy_blue)
...
function noisify(image::AbstractMatrix, s)
	noisy_image = [noisify(i, s) for i in image]
	return noisy_image
end
...
The image is unrecognisable with intensity 0.85.
"""
