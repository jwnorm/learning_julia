...
function extend(v::AbstractVector, i)
	...
	if i in ind
		...
		if i < minimum(1:len)
			closest_edge = minimum(ind)
		else
			closest_edge = maximum(ind)
		end
		component = v[closest_edge]
	end
...
@bind l_box Slider(1:length(v), show_value=true, default=1)
...
function extend(M::AbstractMatrix, i, j)
	...
	if i in row_ind
		...
		if i < minimum(row_ind)
			closest_row = minimum(row_ind)
		else
			closest_row = maximum(row_ind)
		end
	end
	...
	if j in col_ind
		...
		if j < minimum(col_ind)
			closest_col = minimum(col_ind)
		else
			closest_col = maximum(col_ind)
		end
	end

	closest_element = M[closest_row, closest_col]
...
function box_blur(v::AbstractArray, l)
	...
	for i in 1:len
		window = []
		for j in i-l:i+l
			append!(window, extend(v, j))
		end
		pix_mean = mean(window)
		append!(blurred_v, pix_mean)
	end
...
function convolve(v::AbstractVector, k)
	len = length(k)
	offset = len ÷ 2
	k_offset = OffsetArray(k, -offset:offset)

	conv_v = []
	for i in 1:length(v)
		window = []
		for (j, kk) in zip(i-offset:i+offset, k_offset)
			append!(window, extend(v, j) * kk)
		end
		append!(conv_v, sum(window))
	end
...
function convolve(M::AbstractMatrix, K::AbstractMatrix)
	...
	row_index = 1 - row_offset : m_rows + row_offset
	col_index = 1 - col_offset : m_cols + col_offset
	
	ext_m = [extend(M, i, j) for (i,j) in Iterators.product(row_index, col_index)]
	offset_m = OffsetArray(ext_m, row_index, col_index)

	conv_m = Array{Any}(nothing, m_rows, m_cols)
	for row in 1:m_rows
		for col in 1:m_cols
			row_slice = row - row_offset : row + row_offset
			col_slice = col - col_offset : col + col_offset
			sub_m = offset_m[row_slice, col_slice]
			conv_m[row, col] = sum(sub_m .* K)
		end
	end
	
	return Array{Color}(conv_m)
end
...
function with_gaussian_blur(image; σ=3, l=5)
	...
	offset = l ÷ 2
	slice = 0 - offset : 0 + offset
	
	kernel_list = [gauss(i, j, σ=σ) for (i, j) in Iterators.product(slice, slice)]
	kernel_norm = kernel_list ./ sum(kernel_list)

	conv_image = convolve(image, kernel_norm)
