...
float_example = [j for j in 1:1_000 if j*(1/j) != 1]

# ╔═╡ 8efcaaeb-4900-404b-ae59-65db0bde8790
begin
	first_float = float_example[1]
	error = 1 - first_float * (1/first_float)
end
...
function matvec(M::RankOneMatrix, x)
	...
	return [sum(s .* M.v) for s in (M.w .* x)]
end
...
begin
	...
	RankTwoMatrix(v1) = RankTwoMatrix(RankOneMatrix(v1), RankOneMatrix(v1))
...
md"The distance betweeen the Julia computation and 1 is 2 to the power $pow."
...
# ╔═╡ 6399d5f9-dcf5-471d-ae88-1b6acc70afc7
size([1 2 3; 2 2 2 ; 3 3 3], 1)
...
function matvec(M::LowRankMatrix, x)
	...
	return sum([matvec(m, x) for m in M.Ms])
end

# ╔═╡ 2d65bd1a-8512-11eb-1bd2-0313588dfa0e
md"""
#### Exercise 3.7
One of the big advantages of our rank-1 matrices is its space efficiency: to "store" a $n\times n$ matrix, we only need to *actually* store two vectors of $n$ entries, as opposed of $n^2$ total entries. Rank-2 matrices are a little less efficient: we store two rank-1 matrices, or four vectors of $n$ vertices. As we increase the rank, we lose space efficiency, at the cost of being able to represent more kinds of matrices.
...
Because at that $k$, the number of individual entries in the `LowRankMatrix` will be equivalent to the total number of entries in the full $n \times n$ matrix.
"""
...
# ╔═╡ c6a8e87b-6110-48e5-8750-79bf09c8fb55
...
function k_rank_ones(k, m, n)
	...
	return sum(RankOneMatrix(rand(m), rand(n)) for i in 1:k)
end

# ╔═╡ aac78a3c-dd95-4f69-98b6-0dee21c0453e
rand(3, 3)

# ╔═╡ cc9a541f-102f-4303-8096-92ee36e40dc0
k_rank_ones(1, 3, 3)
...
numerical_rank(k_rank_ones(8, 10, 10))

# ╔═╡ 070e5e3a-9641-4a7a-bae7-a1a939ef0f19
numerical_rank(k_rank_ones(20, 10, 10))

# ╔═╡ 57c19601-122b-414f-bc99-56f98c794e61
numerical_rank(k_rank_ones(20, 8, 10))

# ╔═╡ 9813e9f9-9970-4a8b-b3ca-e6a699e6fda4
numerical_rank(k_rank_ones(20, 10, 6))

# ╔═╡ 85873cd2-dfad-48d0-a038-e3cb46bc31dd
md"The numerical rank of a matrix $A$ is limited by the dimensions of $A$. If the number of rank one matrices that composes $A$ (let's call this value $k$) is greater than either $m$ or $n$, the number of essentially non-zero singular values is bounded by the lesser of the two dimensions. Otherwise, the numerical rank is equal to $k$."
