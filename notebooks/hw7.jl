...
# ╔═╡ 37c35551-5ac0-4908-95e7-fb5da9b6e30a
using Statistics
...
# ╔═╡ 9f5aaa97-e2fd-483f-b61d-1cfc51fa9c26
Integer(I)

# ╔═╡ 1529c319-ca4e-431c-8c14-5939e9ce9960
Integer(R)

# ╔═╡ cf2a1092-8169-4991-9c21-a332d19107b9
md"""
> The variable `S` has the value 0. `I`increments by 1 and `R` increments by a subsequent 1.
"""
...
# ╔═╡ 9f62e560-a7fd-405d-9154-44b3d02577c7
Agent()

# ╔═╡ 60c6150a-fe9c-432e-8011-40977c33c32d
methods(Agent)
...
# ╔═╡ 5c98c7ca-b2f0-4c0c-a844-842fdb1daf31
function set_num_infected!(agent::Agent)
...
# ╔═╡ 0aa40102-b386-4fee-beb6-a8f973faedc1
...
function generate_agents(N::Integer)
	...
	# randomly select one agent in range to be infected
	initial_infected = agents[rand(1:N)]
	initial_infected.status = I
...
# ╔═╡ 2bde18ff-c1cf-41d1-b8d1-225fc02f8a0d
function choose_agents(agents::Vector{Agent})
	
	N = length(agents)
	agent_ind = rand(1:N)
	source_ind = rand(1:N)

	# ensure that agent is not acting on itself
	while agent_ind == source_ind
		source_ind = rand(1:N)
	end
...
function plot_infections(simulations)
	p = plot()
	...
	# calculate mean infections at each timestep
	A = reduce(hcat, [i.I for i in simulations])
	I_avg = sum(A, dims=2) ./ size(A, 2)

	plot!(p, 1:1000, I_avg, lw=3, color=:black, label="mean")
	xlabel!("time")
	ylabel!("infectious")
...
# ╔═╡ 363ea404-668e-4dc2-af03-990293cf5995
md"""
> Yes, the mean line does seem to follow the general trend of infections at any given timestep in the above plot.
"""
...
# ╔═╡ ed930c07-6971-4c16-a34b-decd3fda1d32
function calculate_means(simulations::Vector{<:NamedTuple})

	status_ms = Dict()
	for status in keys(first(simulations))
		A = reduce(hcat, [i[status] for i in simulations])
		m = sum(A, dims=2) ./ size(A, 2)
		status_ms[String(status)] = m
	end
...
function sir_mean_plot(simulations::Vector{<:NamedTuple})
	...
	ms = calculate_means(simulations)

	p = plot()
	
	for status in keys(ms)
		if status != "num_infected"
			plot!(p, 1:T, ms[status], label=status, lw=2)
		end
	end
...
md"""
p infection = $@bind p_infection Slider(0:0.0001:0.02, show_value=true, default=0.015)
"""

# ╔═╡ 5fd4b4ed-82f9-46db-83a6-be67912e4465
md"""
p recovery = $@bind p_recovery Slider(0:0.00001:0.002, show_value=true, default=0.0005)
"""
...
# ╔═╡ f612cc1f-c49c-4d0a-9cdd-fcadfce2b087
md"""
> Need to load `Statistics.jl` for `std` function.
"""

# ╔═╡ 24d890f4-8eb1-49a8-a6ef-ce90bbaf7918
function calculate_sigmas(simulations::Vector{<:NamedTuple})

	status_σs = Dict()
	for status in keys(first(simulations))
		A = reduce(hcat, [i[status] for i in simulations])
		σ = mapslices(std, A; dims=2)
		status_σs[String(status)] = σ
	end
...
function sir_mean_error_plot(simulations::Vector{<:NamedTuple})
	...
	T = length(first(simulations).S)
	ms = calculate_means(simulations)
	σs = calculate_sigmas(simulations)

	p = plot()
	
	for status in keys(ms)
		if status != "num_infected"
			plot!(p, 1:T, ribbon=σs[status], 
				  ms[status], label=status, lw=2)
		end
	end
...
# ╔═╡ 47211167-30e9-40c2-a076-96dc88190c83
md"""
> Let's bring in a couple helper functions from the last homework assignment, `counts` and `counts2`, in order to calculate the probability distribution of the `num_infected` for the simulation run.
"""

# ╔═╡ 9787fb23-853a-42e6-9521-7a003732e065
...
# ╔═╡ 440ebf65-21f8-4398-b3c8-f8d1249f2908
function count2(data::Vector)
	dict = counts(data)
...
# ╔═╡ 67ee416a-5e94-4843-900b-9625fe0552d9
md"""
> With that initial setup out of the way, let's run our `simulation` method. To ensure we are getting a good visual, I am increasing `p_infection` to 0.5.
"""

# ╔═╡ 83558982-ef4d-4ba1-8062-692154060cae
md"""
> We need to fill the individual vectors to ensure they are the same length. We will do this by appending zeros to the end of each one until we reach the required length. In this step, we will also convert our absolute frequencies to probabilities.
"""

# ╔═╡ e8f19587-71cf-4917-ac6b-931a3ca7b944
md"""
> And finally, for the plot:
"""

# ╔═╡ 7621420c-1149-47ba-a12e-271d19059b4e
md"""
> The behavior of this probability distribution as it evolves through time is very similar to the Pascal's Triangle visual we saw previously. It seems to be converging to a geometric distribution.
"""
...
function step!(agents::Vector{Agent}, infection::AbstractInfection)
	...
	agent, source = choose_agents(agents)
	interact!(agent, source, infection)
...
# ╔═╡ abb673a1-4d0b-4d9b-95af-f37d9d2ac68b
plot_infections(simulations)
...
# ╔═╡ d376739b-f842-4213-944a-3a27c6b08d77
sir_mean_error_plot(simulations)

# ╔═╡ 0668fe4b-122b-4ec1-acd4-4cc86a5c862c
let
	sims = repeat_simulations(100, 1000, 
		   InfectionRecovery(p_infection, p_recovery), 20)
	sir_mean_plot(sims)
end

# ╔═╡ 9bab800e-ae6b-4a7d-b188-f29f5c928e3d
sim = simulation(1000, 50, InfectionRecovery(0.5, 0.01));

# ╔═╡ 37392897-6d79-4169-a698-f23d9b509d62
freqs = hcat([count2(i)[2] for i in sim.num_infected])

# ╔═╡ 9a54b957-5233-4931-98b9-68761b69fbe1
begin
	max_num_infected = length(last(freqs))
	fs = []
	
	for f in freqs
		gap = max_num_infected - length(f)
		line = Integer.(vcat(f, zeros(gap)))
		push!(fs, line)
	end
	ps = [i ./ sum(i) for i in fs]
	ylabels = 0:max_num_infected - 1;
end

# ╔═╡ 2d603569-3b05-416a-bb84-a051b6276ed8
let
	...
	plot!(xs, ys, zs, c=cs, xlims=(0, 50), ylims=(-10, 10), zlims=(0, 1), lw=2.5, alpha=0.7, xticks=(0:10:50), yticks=(-10:5:10))
...
begin
	test_sim = simulation(1000, 10_000, InfectionRecovery(0.02, 0.001))
	
	plot(test_sim.S, label="S", lw=2)
	plot!(test_sim.I, label="I", lw=2)
	plot!(test_sim.R, label="R", lw=2)

	xlabel!("time")
	ylabel!("# agents")
end

# ╔═╡ d65d87a4-fe15-436d-9b39-1ef741d41ad1
begin
	peak_num, peak_time = findmax(test_sim.I)
	slope = maximum([test_sim.I[i] - test_sim.I[max(i-1, 1)]
					for i in 1:length(test_sim.I)])

md"""
1. Max concurrent infected = $(peak_num / 10)%
2. Time to peak infectious = $(peak_time) timesteps
3. Max infections in one timestep = $(slope) infections
"""
end
...
Note that you should be able to re-use the `sweep!` and `simulation` functions , since those should be sufficiently **generic** to work with the new `interact!` function! (Modify them if they are not.)
...
begin
	reinfection_sim = repeat_simulations(100, 1000, Reinfection(0.02, 0.002), 20);
	plot_infections(reinfection_sim)
end

# ╔═╡ 391c3c0f-ef5d-4c95-a6eb-de807273a7b0
md"""
> The initial stage of the model is similar in both scenarios. There is an exponential growth in the number of infectious persons up until around time 300.
>
> After this point, the models diverge. Our *reinfection* model increases at a decreasing rate for the remainder of our timesteps and ultimately never "peaks;" it converges around 80 to 100 infectious. Alternatively, the *recovery* model begins to slow down, peak, and eventually decline somewhat linearly. The peak number of infectious persons is between 50 and 60.
"""
